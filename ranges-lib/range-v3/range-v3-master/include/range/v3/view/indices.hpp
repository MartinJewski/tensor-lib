...
namespace ranges
{
    namespace views
    {
        ...
        {
            constexpr indices_fn() = default;

            template<typename Val>
            constexpr auto operator()(Val to) const -> CPP_ret(iota_view<Val, Val>)( //
                requires integral<Val>)
            {
                return {Val(), to};
            }
            template<typename Val>
            constexpr auto operator()(Val from, Val to) const -> CPP_ret(iota_view<Val, Val>)( //
                requires integral<Val>)
            {
                return {from, to};
            }
        ...
        {
            template<typename Val>
            constexpr auto operator()(Val to) const -> CPP_ret(closed_iota_view<Val>)( //
                requires integral<Val>)
            {
                return {Val(), to};
            }
            template<typename Val>
            constexpr auto operator()(Val from, Val to) const -> CPP_ret(closed_iota_view<Val>)( //
                requires integral<Val>)
            {
                return {from, to};
            }
